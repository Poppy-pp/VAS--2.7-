...
<template>
	<section>
		...
		<el-table stripe border :data="listData" highlight-current-row style="width: 100%;" :max-height="windowOutHeight-220" ref="listTable">
			...
			<el-table-column prop="MODEL" label="车型" align="center" width="140">
			</el-table-column>
			...
			<el-table-column prop="POSITION" label="车辆位置" align="center" width="230">
			</el-table-column>
			<el-table-column prop="GPSTIME" label="GPS时间" align="center" :formatter="dateFormatter1" width="140">
			</el-table-column>
			<el-table-column prop="RECVTIME" label="服务器时间" align="center" :formatter="dateFormatter2" width="140">
			</el-table-column>
			...
			<el-table-column prop="ALARMCONTENT" label="报警类型" align="center" width="100">
			</el-table-column>
			<el-table-column prop="RECVTIME" label="报警时间" align="center" :formatter="dateFormatter3" >
			</el-table-column>
		</el-table>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/*导出*/
			handleExport() {
				window.location.href = '/vasms-web/api/v1/bi/biAfterSale/export_alarm?_viewName=VW_BI_AFTERSALE_ALARM&token=' + this.$store.getters.userInfo.token + '&corpname=' + this.filters.corpname + '&searchKey=' + this.filters.searchKey + '&showCount=' + 20000;
			},
			/* 执行最新记录信息*/
			handleImplement() {
				this.$confirm('此操作将刷新 【报警报表】信息，是否刷新？', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					// let para = {
					// 	p_name: 'p_calculate_aftersale_alarm',
					// 	params: this.filters.recMonth,
					// };
					let loading = util.startLoading(this,'.el-table__body-wrapper');//调用加载
					axios.get('/vasms-web/api/v1/bi/biAfterSale/callP_alarm').then((res) => {
			...
			getData() {
				...
				let loading = util.startLoading(this,'.el-table__body-wrapper');//调用加载
				axios.get('/vasms-web/api/v1/bi/biAfterSale/getAlarm', {params: para}).then((res) => {
			...
			dateFormatter2(row, col) {
				if(row.RECVTIME == undefined) return '--';
				return util.formatDate.format(new Date(row.RECVTIME), 'yyyy-MM-dd hh:mm:ss');
			},
			dateFormatter3(row, col) {
				if(row.RECVTIME == undefined) return '--';
				return util.formatDate.format(new Date(row.RECVTIME), 'yyyy-MM-dd');
			},
			/*所属公司——下拉*/
		            corpChange(r) {
