...
<template>
	<section class="tab_content-wrapper yewu">
		<div v-show="formDialogTableVisibleCommon">
			...
			<el-table stripe :max-height="windowOutHeight-180" :data="todo" border highlight-current-row v-loading="listLoading" @row-dblclick="showDetial" style="width: 100%;">
				...
				<el-table-column prop="cur_name" align="center" label="当前状态">
					<template slot-scope="props">
						<el-tag type="primary" style="font-weight:bold;">{{ props.row.cur_name }}</el-tag>
					</template>
				</el-table-column>
				...
				<el-table-column prop="0" align="center" width="170" label="申请时间">
				</el-table-column>
...
<script>
	...
	import { queryRequest, getvariablesTaskInfoList, getvariablesSeaviceTaskInfoList,getDeptManagerInfoList,getCorpNameList } from '../../api/api';
	export default {
		...
		methods: {
			...
			orderFormatter(row, col) {
				if(row.proc_def_id == "proc_vindicate_remove:1:667504") {
					return "拆除";
				}
				if(row.proc_def_id == "proc_vindicate:5:495004") {
					return "售后维修";
				}
				if(row.proc_def_id == "proc_new_build:7:557589") {
					return "新装";
				}
			},
			//获取我的待办列表列表
