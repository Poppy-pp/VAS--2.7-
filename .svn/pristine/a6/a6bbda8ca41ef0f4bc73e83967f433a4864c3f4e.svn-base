...
<template>
	<section class="tab_content-wrapper">
						...
						<div style="display:inline-block;margin:0 10px 10px 0;">
							<el-input class="noborder color icon nofocus" @keyup.native.ctrl.8="clearAll()" @keyup.native.13="handleQuerySelect" placeholder="请输入查询内容" v-model="filters.domSearch[index].content">
								<el-select class="wp_select" multiple clearable filterable v-model="filters.domSearch[index].select" slot="prepend" placeholder="选择条件">
									...
									<el-option label="设备号" value="prodnum"></el-option>
									<el-option label="ICCID卡号" value="simnum"></el-option>
									<el-option label="通讯号" value="mobile"></el-option>
								</el-select>
		...
		<!--列表-->
		<el-table :max-height="windowOutHeight-200" stripe border :data="listData" highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="simsnnum" label="ICCID卡号" align="center" >
			</el-table-column>
			...
			<el-tabs>
				...
				<el-row style="padding:10px 40px;margin-bottom:20px;border-bottom:1px solid #C4E8D6;" v-show="this.prodThisList.simID != undefined">
					...
					<el-col :span="12">
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>ICCID卡号:</dt>
							<dd>{{ prodThisList.simprodsnnum }}</dd>
						</dl>
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>卡类型:</dt>
					...
					<el-col :span="24">
						<el-form-item id="nomargin">
							<el-table :data="simdatas" empty-text="暂未绑定卡" border style="width: 100%" max-height="250">
								<el-table-column prop="prodsnnum" label="ICCID卡号" align="center">
								</el-table-column>
								...
								<el-table-column label="操作" width="80" align="center">
									<template scope="scope">
										<el-button type="danger" size="small" icon="el-icon-share" class="mt5" @click.native="cancelSim(scope.$index,vehdatas)">解绑</el-button>
									</template>
								</el-table-column>
							</el-table>
							<el-button type="primary" @click.native="addDeviceListSim" icon="el-icon-plus" style="margin-top:10px;">绑定SIM卡</el-button>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
		...
		<el-dialog title="SIM卡信息" :modal-append-to-body="false" :visible.sync="deviceFormVisible" top="3%" @close="clearAll()" :close-on-click-modal="false">
							...
							<div style="display:inline-block;margin:0 10px 10px 0;">
								<el-input class="noborder color icon nofocus" @keyup.native.ctrl.8="clearAll()" @keyup.native.13="getDeciceListInfo" placeholder="请输入查询内容" v-model="filters.domSearch[index].content">
									<el-select class="wp_select" multiple clearable filterable v-model="filters.domSearch[index].select" slot="prepend" placeholder="选择条件">
										...
										<!-- <el-option label="卡号" value="prodnum"></el-option> -->
									</el-select>
									<template v-if="index == filters.domSearch.length-1">
										<el-button slot="append" @click="addSelect" icon="el-icon-plus" title="添加查询条件"></el-button>
									</template>
			...
			<!--列表-->
			<el-table border :data="deviceListData" max-height="500" ref="deviceListData" highlight-current-row v-loading="devicelistLoading" @select="selsChangeProd" style="width: 100%;">
			...
			<!--工具条-->
			<el-col :span="24" class="toolbar">
				<el-button type="primary" icon="el-icon-share" class="mt5" @click="comlistProd" :loading="comfirLoadingSIM" :disabled="this.sels.length===0">确认绑卡</el-button>
				...
				<el-row :gutter="20">
					<el-col :span="24">
						<el-form-item id="nomargin">
							...
							<el-button type="primary" @click.native="addDeviceListVeh" icon="el-icon-plus" style="margin-top:10px;">绑定车辆</el-button>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
		...
		<!--设备绑车——车辆列表选择界面-->
		<el-dialog title="车辆信息" :modal-append-to-body="false" :visible.sync="deviceFormVisibleVeh" top="3%" :close-on-click-modal="false" @close="clearAll()">
			<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
				<el-form :inline="true" :model="filters">
					<template v-for="(item,index) in filters.domSearch">
						<template v-if="index == 0">
			...
			<!--选择车辆列表-->
			<el-table border :data="deviceListDataVeh" max-height="500" ref="deviceListDataVeh" highlight-current-row v-loading="devicelistLoadingProd" @select="selsChangeVeh" style="width: 100%;">
				<el-table-column type="selection" fixed="left" width="50" align="center">
				</el-table-column>
				<el-table-column prop="licenseplatenum" label="车牌号" align="center">
				</el-table-column>
	...
	export default {
		...
		methods: {
			...
			comlistVeh() {
				...
				let para = {
					vehicleid: this.checkDataVeh.ID,
					packinfos: [{
			...
			selsChangeVeh(sels, row) {
				this.$refs.deviceListDataVeh.clearSelection(); //清除其他选项框
				this.$refs.deviceListDataVeh.toggleRowSelection(row, true); //选中选项框
				this.sels = sels;
			...
			comlistProd() {
				...
				let para = {
					equipmentid: this.bindDataRow.prodid,
					simcardid: this.checkDataProd.id,
					storageid: this.bindDataRow.storageid,
				};
			...
			getDeciceListInfo() {
				if(this.bindDataRow.storageid == undefined) {
					...
					let para = {
						...
						ispack: 0,
						isdelete: 0,
						// storageid:this.prodThisList.storageid,
						prodcategory: 'C',
						status: 'INSTO',
						storageid: this.bindDataRow.storageid,
					};
