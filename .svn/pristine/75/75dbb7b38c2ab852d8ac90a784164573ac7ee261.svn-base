...
<template>
	<section class="tab_content-wrapper">
						...
						<div style="display:inline-block;margin:0 10px 10px 0;">
							<el-input class="noborder color icon nofocus" @keyup.native.ctrl.8="clearAll()" @keyup.native.13="handleQuerySelect" placeholder="请输入查询内容" v-model="filters.domSearch[index].content">
								<el-select class="wp_select" multiple clearable filterable v-model="filters.domSearch[index].select" slot="prepend" placeholder="选择条件">
									<el-option label="车载终端协议名称" value="protocolname"></el-option>
								</el-select>
		...
		<el-table :max-height="windowOutHeight-170" border stripe :data="listData" :row-class-name="tableRowClassName" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;" @sort-change="sortChange">
			...
			<el-table-column prop="protocolcode" label="车载终端协议代码" align="center" >
			</el-table-column>
			<el-table-column prop="protocolname" label="车载终端协议名称" align="center" >
			</el-table-column>
		...
		<el-dialog title="" :modal-append-to-body="false" :visible.sync="formDialogTableVisible" size="small">
			<el-tabs>
				<el-row style="padding:0 40px;margin-bottom:20px;border-bottom:1px solid #C4E8D6;">
					<el-col :span="24">
						<span class="formTile">协议信息</span>
					</el-col>
					<el-col :span="24">
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>车载终端协议代码:</dt>
							<dd>{{ agreementsThisList.protocolcode }}</dd>
						</dl>
					</el-col>
					<el-col :span="24">
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>车载终端协议名称:</dt>
							<dd>{{ agreementsThisList.protocolname }}</dd>
						</dl>
					</el-col>
				</el-row>
				...
				<el-row style="padding:10px 40px;margin-bottom:20px;border-bottom:1px solid #C4E8D6;">
					...
					<el-col :span="5">
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>创建人:</dt>
							<dd>{{agreementsThisList.createby}}</dd>
						</dl>
					</el-col>
					<el-col :span="7">
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>创建日期:</dt>
							<dd>{{agreementsThisList.createdate == null ? "" :fmtdata.formatDate.format(new Date(agreementsThisList.createdate), 'yyyy-MM-dd') }}</dd>
						</dl>
					</el-col>
					<el-col :span="5">
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>修改人:</dt>
							<dd>{{agreementsThisList.updateby}}</dd>
						</dl>
					</el-col>
					<el-col :span="7">
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>修改日期:</dt>
							<dd>{{agreementsThisList.updatedate == null ? "" :fmtdata.formatDate.format(new Date(agreementsThisList.updatedate), 'yyyy-MM-dd') }}</dd>
						</dl>
					</el-col>
				</el-row>
			</el-tabs>
		...
		<el-dialog title="编辑" :modal-append-to-body="false" :visible.sync="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="150px" :rules="editFormRules" ref="editForm">
				<el-row :gutter="20">
					<el-col :span="24">
						<el-form-item label="车载终端协议代码" prop="protocolcode">
							<el-input v-model="editForm.protocolcode" auto-complete="off"></el-input>
						</el-form-item>
						<el-form-item label="车载终端协议名称" prop="protocolname">
							<el-input v-model="editForm.protocolname" auto-complete="off"></el-input>
						</el-form-item>
			...
			<el-form :model="addForm" label-width="150px" :rules="addFormRules" ref="addForm">
				<el-row :gutter="20">
					<el-col :span="24">
						...
						<el-form-item label="车载终端协议名称" prop="protocolname">
							<el-input v-model="addForm.protocolname" auto-complete="off"></el-input>
						</el-form-item>
						<el-form-item label="备注" prop="remarks">
...
<script>
	...
	import { getProtocolList, addProtocol, modifyProtocol, removeProtocol, getSelectListProtocol } from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			//详情查看
			formDetailHandle(agreementsID) {
				this.formDialogTableVisible = true;
				this.agreementsThisList = ''; //清空上条数据
				// 获取当前分组信息
				let para = {
					id: agreementsID
				}
				...
				getProtocolList(para).then((res) => {
					this.agreementsThisList = res.data.data.records[0];
					nanobar.go(100);
				});
			},
			// 有效 鼠标移入
			...
			handleQuerySelect() {
				...
				getSelectListProtocol(para).then((res) => {
			...
			editSubmit() {
				this.$refs.editForm.validate((valid) => {
					if(valid) {
						...
						let para = {
							id: this.editForm.id,
							protocolcode: this.editForm.protocolcode,
			...
			addSubmit() {
				this.$refs.addForm.validate((valid) => {
					if(valid) {
						...
						let para = {
							protocolcode: this.addForm.protocolcode,
							protocolname: this.addForm.protocolname,
							remarks: this.addForm.remarks,
							isactive: this.addForm.isactive,
						}
						addProtocol(para).then((res) => {
