<template>
	<section class="tab_content-wrapper">
		...
		<el-table border :max-height="windowOutHeight-170" stripe :data="listData" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column label="操作" align="center" width="220">
				<template scope="scope">
					...
					<el-button id="button" @click="roleEdit(scope.$index, scope.row)" title="编辑角色" :disabled="scope.row.isenable == '0' ? true : false">
					...
					<el-button id="button" @click="groupEdit(scope.$index, scope.row)" title="编辑分组权限" :disabled="scope.row.isenable == '0' ? true : false">
					...
					<el-button id="button" @click="corpEdit(scope.$index, scope.row)" title="编辑组织权限" :disabled="scope.row.isenable == '0' ? true : false">
		...
		<el-col :span="24" class="toolbar">
			...
			<el-pagination layout="prev, pager, next" background @current-change="handleCurrentChange" :page-size="20" :total="total" style="margin-top:5px;float:right;">
			</el-pagination>
		</el-col>
		...
		<el-dialog title="编辑" :modal-append-to-body="false" :visible.sync="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="用户名" prop="name" ref="name">
					<el-input v-model="editForm.name" auto-complete="off" @blur="checkout('name',editForm.name,0)"></el-input>
				</el-form-item>
		...
		<!--组织权限编辑界面-->
		<el-dialog title="编辑当前用户可操作的分组" :modal-append-to-body="false" :visible.sync="editGroupInfoVisible" :close-on-click-modal="false" @close="groupEditClose">
			<el-input style="margin-bottom:10px" placeholder="输入关键字查找分组" v-model="filterTextGroup"></el-input>
			<el-tree v-loading="treeRouseGroupLoading" element-loading-text="分组树列表加载中..." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.7)" :data="groupRoleList" show-checkbox highlight-current node-key="id" check-strictly :default-expanded-keys="defaultExpandedKeysGroup" :default-checked-keys="defaultCheckedKeysGroup" ref="treeRouseGroup" :render-content="renderContentGroup" @check-change="handleCheckChangeGroup" :props="defaultPropsGroup" :filter-node-method="filterNodeGroup">
			</el-tree>
			<div slot="footer" class="dialog-footer">
		...
		<el-dialog title="编辑当前用户可操作的组织" :modal-append-to-body="false" :visible.sync="editCorpInfoVisible" :close-on-click-modal="false" @close="corpEditClose">
			<el-tabs v-model="activeName" type="card" @tab-click="tabClick">
				<el-tab-pane label="VAS-平台管理系统" name="first">
					...
					<el-tree v-loading="treeRouseVasLoading" element-loading-text="VAS树列表加载中..." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.7)" :data="corpRoleList" show-checkbox highlight-current node-key="id" check-strictly :default-expanded-keys="defaultExpandedKeys" :default-checked-keys="defaultCheckedKeys" ref="treeRouseVas" :render-content="renderContentVas" @check-change="handleCheckChangeVas" :props="defaultProps" :filter-node-method="filterNodeCorp">
					</el-tree>
				</el-tab-pane>
				<el-tab-pane label="GIS-车辆位置服务系统" name="second" :disabled="treeRouseVasLoading">
					...
					<el-tree v-loading="treeRouseLoading" element-loading-text="GIS树列表加载中..." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.7)" :data="gisCorpRoleList" show-checkbox highlight-current node-key="id" check-strictly :default-expanded-keys="gisDefaultExpandedKeys" :default-checked-keys="gisDefaultCheckedKeys" ref="treeRouse" :render-content="renderContent" @check-change="handleCheckChange" :props="defaultProps" :filter-node-method="filterNodeCorp">
					</el-tree>
				</el-tab-pane>
			</el-tabs>
			<div slot="footer" class="dialog-footer">
		...
		<!---->
		<el-dialog title="编辑当前用户可操作的车辆" :modal-append-to-body="false" :visible.sync="editLabelVehInfoVisible" :close-on-click-modal="false" @close="vehEditLabelClose">
			...
			<div slot="footer" class="dialog-footer">
				<el-checkbox v-model="checkAllTwoLable" @change="handleCheckAllChangeTwoLabel" style="float:left;" :disabled="checkAllThisPageLabel">全选</el-checkbox>
				<el-checkbox id="checkThisPage" v-model="checkAllThisPageLabel" @change="handleCheckAllChangeThisPageLabel" style="float:left;" :disabled="checkAllTwoLable">选择当页</el-checkbox>
...
<script>
	...
	import { getSysUserInfoList, addSysUserInfo, modifySysUserInfo, removeSysUserInfo, getSysRoleInfoList, editUserRole, getUserRole, getSysUserList, getRuleResourceInfo, getCorpAllList, editCorpRole, getThisCorpList, getCheckoutOfUser, getSelectListVehnotTag, getHaveVehList, modifyHaveVehList, getGroupAllList, getGroupThisList, editGroupRole,corporatelevel_authority,corporatevehicle_vehicles,getSelectListVehnotAuthority } from '../../api/api';
	export default {
			...
			labelRenderContent(h, {
				...
				if(data.parentcorpcode != '0') { //当不为第一级【类别】的时候
					str = (<span>
                        ...
                        <el-button style="margin-left:10px;display: inline-block;padding: 0 5px;height: 24px;line-height: 22px;font-size: 12px;opacity:0.8;" id={'ltag'+data.id} size='mini' on-click={(e)=>{this.vehLabelEdit(data);e.stopPropagation();}}>车辆权限分配</el-button>
                    </span>);
					return str;
				} else { //当为第一级的时候
			...
			renderContent(h, {
				...
				if(data.parentcorpcode != '0') { //当不为第一级【类别】的时候
					...
					str = (<span>
                        ...
                        { (status == 'SUB')
                        ? (<el-button style="margin-left:10px;display: inline-block;padding: 0 5px;height: 24px;line-height: 22px;font-size: 12px;opacity:0.8;" id={'tag'+data.id} type="warning" on-click={(e)=>{this.vehEdit(data);e.stopPropagation();}}>部分</el-button>) 
                            : (<el-button style="margin-left:10px;display: inline-block;padding: 0 5px;height: 24px;line-height: 22px;font-size: 12px;opacity:0.8;" id={'tag'+data.id} type="success" on-click={(e)=>{this.vehEdit(data);e.stopPropagation();}}>全部</el-button>)
                        }
                    </span>);
					return str;
				} else { //当为第一级的时候
