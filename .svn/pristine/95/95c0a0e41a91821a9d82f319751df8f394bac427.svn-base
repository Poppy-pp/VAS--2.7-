...
<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<!-- <el-form-item prop="month">
					<el-date-picker @change="changeTime" :picker-options="pickerOptions" v-model="month" type="month" placeholder="请选择月份"></el-date-picker>
				</el-form-item> -->
				<el-button-group>
		...
		<el-table stripe border :data="listData" highlight-current-row style="width: 100%;" :max-height="windowOutHeight-220" ref="listTable" show-summary>
			...
			<el-table-column :label="'超 ' + this.overMonth +' 库存'" align="center">
				<el-table-column prop="REC_AREA" label="地区" align="center"></el-table-column>
				<el-table-column prop="PRODCOUNT" label="设备数量" align="center"></el-table-column>
			</el-table-column>
		</el-table>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/*导出*/
			handleExport() {
				window.location.href = '/vasms-web/api/v1/bi/biViewNameAll/export?_viewName=VW_BI_OLDPROD_BALANCE&token=' + this.$store.getters.userInfo.token + '&searchKey=' + this.filters.searchKey + '&showCount=' + 20000;
			},
			// 重置
			resetForm(){
			...
			getData() {
				...
				axios.get('/vasms-web/api/v1/bi/biViewNameAll?_viewName=VW_BI_OLDPROD_BALANCE', {params: para}).then((res) => {
			...
			getOverMonth(){
				var date = new Date();
				this.overMonth = util.formatDate.format(new Date(date.setMonth( date.getMonth()-2 )), 'yyyy-MM-dd');
			},
