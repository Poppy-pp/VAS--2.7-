...
<template>
	<section>
		...
		<el-table stripe border :data="listData" highlight-current-row style="width: 100%;" :max-height="windowOutHeight-220" ref="listTable">
			...
			<el-table-column prop="" label="风控情况确认" align="center">
			</el-table-column>
			<el-table-column prop="" label="资管情况确认" align="center">
			</el-table-column>
			<el-table-column prop="" label="风控确认是否结款" align="center">
			</el-table-column>
			<el-table-column prop="" label="万网处理分类" align="center">
			</el-table-column>
			<el-table-column prop="" label="新装/售后处理结果" align="center">
			</el-table-column>
			<el-table-column prop="" label="无法处理类别" align="center">
			</el-table-column>
			<el-table-column prop="" label="新装/售后处理详情" align="center">
			</el-table-column>
		</el-table>
...
<script>
	...
	export default {
		...
		methods: {
			/*导出*/
			handleExport() {
				window.location.href = '/vasms-web/api/v1/bi/biInstallDetail/export?_viewName=VW_BI_INSTALLDETAIL&token=' + this.$store.getters.userInfo.token + '&searchKey=' + this.filters.searchKey + '&vehiclestate=' + this.filters.vehiclestate + '&corpid=' + this.filters.corpid + '&newflag=' + 1 + '&showCount=' + 20000
			...
			handleImplement() {
				...
				this.$confirm('此操作将刷新 ' + util.formatDate.format(new Date(this.filters.startDate[0]), 'yyyy-MM') + '~' + util.formatDate.format(new Date(this.filters.startDate[1]), 'yyyy-MM') + ' 信息，是否刷新？', '提示', {
					...
					axios.get('/vasms-web/api/v1/bi/biViewNameAll/callp', {params: para}).then((res) => {
						this.getData();
						this.$message({
							message: "刷新成功！",
							type: 'success'
