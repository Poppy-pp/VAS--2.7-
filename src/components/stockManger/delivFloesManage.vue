...
<template>
	<section class="tab_content-wrapper">
		...
		<el-table border :max-height="windowOutHeight-175" stripe :data="listData" highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="PRODUCT" label="发货明细" align="center" width="100">
				<template scope="scope">
					<!--列表-->
			...
			<el-table-column fixed="right" label="操作" width="250" align="center">
				<template scope="scope">
					...
					<el-button size="small" type="primary" @click="comfirmDeliver(scope.$index, scope.row)" :disabled="((scope.row.DELIVERYSTATUS == '3' && scope.row.DELIVERYSTATUS != undefined) || scope.row.DELIVERYSTATUS == '7') 
					|| (scope.row.DELIVERYCATEGORY == '6' || scope.row.DELIVERYCATEGORY == '7')">确认收货</el-button>
					<el-button size="small" @click="backDeliver(scope.$index, scope.row)" :disabled="(scope.row.DELIVERYSTATUS == '3' && scope.row.DELIVERYSTATUS != undefined) || scope.row.DELIVERYSTATUS == '4' || scope.row.DELIVERYSTATUS == '7' || scope.row.DELIVERYCATEGORY == '6' || scope.row.DELIVERYCATEGORY == '7'">退回</el-button>
				</template>
			</el-table-column>
		</el-table>
