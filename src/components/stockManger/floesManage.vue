...
<template>
	<section class="tab_content-wrapper">
		...
		<el-table border :max-height="windowOutHeight-175" stripe :data="listData" highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="RECEIVEDATE" width="100" label="到货时间" :formatter="dateFormatter2" align="center">
			</el-table-column>
			...
			<el-table-column prop="PRODUCT" label="发货明细" align="center" width="80">
				<template scope="scope">
					<el-popover ref="popover4" placement="right" width="700" @hide="hideprodData" @show="showprodData(scope.row.ID)" trigger="hover">
						<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
						...
						<el-table max-height="400" border :data="prodData">
							<el-table-column type="index" label="#" width="25" align="center">
							</el-table-column>
							<el-table-column prop="EPRODNUM" label="自编号" align="center"></el-table-column>
					...
					<a href="javascript:void(0);" @click.stop.prevent="" v-popover:popover4>
			...
			<el-table-column fixed="right" label="操作" width="250" align="center">
				<template scope="scope">
					...
					<el-button size="small" type="primary" @click="comfirmDeliver(scope.$index, scope.row)" :disabled="((scope.row.DELIVERYSTATUS == '3' && scope.row.DELIVERYSTATUS != undefined) || scope.row.DELIVERYSTATUS == '7' || scope.row.DELIVERYSTATUS == '4') 
					|| (scope.row.DELIVERYCATEGORY == '6' || scope.row.DELIVERYCATEGORY == '7')">确认收货</el-button>
		...
		<!-- 拒绝处理备注 弹窗  start-->
		<el-dialog title="退回原因" :modal-append-to-body="false" :visible.sync="rejectDialogVisible" :close-on-click-modal="false">
			<el-form label-position="left" label-width="80px">
				<el-input type="textarea" autosize placeholder="请输入退回原因" v-model="deliveryreason"></el-input>
				<el-input class="mt10" type="textarea" autosize placeholder="请输入备注" v-model="remarks"></el-input>
			</el-form>
...
<script>
	...
	import { adddelivery, getdoneselfreciveList, modifyCurUserDeviceInfo, backDelivery, getsfhProList } from '../../api/api';
	export default {
		...
		methods: {
			...
			handleQuery() {
				...
				getdoneselfreciveList(para).then((res) => {
