	...
	<section class="tab_content-wrapper">
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.STORAGENAME" placeholder="所属库房"></el-input>
				</el-form-item>
				<el-form-item>
					<el-input v-model="filters.PRODMODEL" placeholder="产品型号"></el-input>
				</el-form-item>
				<el-form-item>
		...
		<el-table border :max-height="windowOutHeight-175" stripe :data="listData" @cell-click="editLimitfunction" :row-class-name="tableRowClassName" highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="" label="当前库存数量" align="center">
				<template scope="scope">
					<el-popover ref="popover4" placement="right" width="600" @show="showprodData(scope.row.PRODUCTMODELID,scope.row.STORAGEID)" trigger="click">
			...
			<!-- <el-table-column label="操作" width="150" align="center">
				<template scope="scope">
					<el-button size="small" @click="handleEdit(scope.$index, scope.row)" :disabled="scope.row.isdelete== '1' ? true : false">编辑</el-button>
					<el-button size="small" v-if="scope.row.isdelete != undefined" :type="scope.row.isdelete == '0'?'success':'warning'" @click="handleChange(scope.$index, scope.row)"> {{scope.row.isdelete == "1" ?"无效":"有效" }} </el-button>
		...
		<el-dialog title="编辑" :modal-append-to-body="false" :visible.sync="editlimtVisible" :close-on-click-modal="false" style="width:800px;margin:0px auto;">
			<el-form :model="editlmitForm" label-width="100px" :rules="editlimtRules" ref="editlmitForm">
...
<script>
	...
	import { getMyStockList, addStockList, modifyStockList, removeStockList, getProdIDInfoList } from '../../api/api';
	...
	export default {
		...
		data() {
			...
			return {
				...
				editlimtRules: {
					...
					maxlimit:[
						{required:true,message:"请输入最高预警值",trigger: 'blur'},
					            {validator: checkNum,trigger: 'blur'}
					],
				},
				//编辑界面数据
		...
		methods: {
			...
			dhandleCurrentChange(val) {
				...
				this.showprodData(this.prodid, this.stoid);
			},
			//切换每页显示数量
