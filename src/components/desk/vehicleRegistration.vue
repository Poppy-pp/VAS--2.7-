							...
							<el-col :span="12">
								...
								<el-form-item label="颜色" prop="colorname">
									<div style="display:flex;">
										<el-select v-model="addForm.colorname" clearable style="flex:1;" placeholder="请选择车辆颜色" value-key="value" @change="changeCarColorAdd" >
								...
								<el-form-item label="车类型" prop="vehicletype">
									<el-select v-model="addForm.vehicletype" @visible-change="vehChange" :loading="vehLoading" filterable placeholder="请选择车类型" clearable>
										<el-option v-for="item in vehlist" :key="item.typedesc" :label="item.typedesc" :value="item.id">
										</el-option>
									</el-select>
								</el-form-item>
...
<script>
	...
	import { getOwnerInfoList, getVehList, getCorpList, getVehicleProd, getPlatList, addVehOwnerList, bandVehicleProd, unBandVehicleProd, addPlatProd, getModelListInfo, getModelListInfo1, getCheckoutOfVehicleInfo,getCorpNameList,getAllColorList } from '../../api/api';
	...
	export default {
		data() {
			...
			return {
				...
				addLoading: false,
				addFormRules: {
					name: [{required: true,message: '请输入车主',trigger: 'blur'}],
					vin: [{
							required: true,
							message: "请输入车架号",
							trigger: 'blur'
