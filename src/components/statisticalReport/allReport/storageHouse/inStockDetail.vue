...
<template>
	<section>
		...
		<el-table stripe border :data="listData" highlight-current-row  style="width: 100%;" :max-height="windowOutHeight-220" ref="listTable">
			...
			<el-table-column prop="UNITPRICE" label="单价" align="center" :formatter="saveNum1">
			</el-table-column>
			<el-table-column prop="TOTALPRICE" label="金额" align="center" :formatter="saveNum2">
			</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/*导出*/
			handleExport() {
				window.location.href = '/vasms-web/api/v1/bi/biViewNameAll/export?_viewName=VW_BI_STO_IN_SUMMARY&token=' + this.$store.getters.userInfo.token + '&searchKey=' + this.filters.searchKey + '&month=' + this.filters.recMonth + '&showCount=' + 20000;
			},
			// 重置
			resetForm(){
			...
			getData() {
				...
				axios.get('/vasms-web/api/v1/bi/biViewNameAll?_viewName=VW_BI_STO_IN_SUMMARY', {params: para}).then((res) => {
			...
			dateFormatter(row, col) {
				if(row.INMONTH == undefined) return '--';
				return util.formatDate.format(new Date(row.INMONTH), 'yyyy-MM-dd');
			},
			...
			saveNum1(row, col) {//保留两位小数
				if (row.UNITPRICE == undefined)return;
