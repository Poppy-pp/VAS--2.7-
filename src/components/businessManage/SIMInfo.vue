						...
						<div style="display:inline-block;margin:0 10px 10px 0;">
							<el-input class="noborder color icon nofocus" @keyup.native.ctrl.8="clearAll()" @keyup.native.13="handleQuerySelect" placeholder="请输入查询内容" v-model="filters.domSearch[index].content">
								<el-select class="wp_select" multiple clearable filterable v-model="filters.domSearch[index].select" slot="prepend" placeholder="选择条件">
									...
									<el-option label="IMEI" value="prodsnnum"></el-option>
			...
			<el-tabs>
				<el-row style="padding:0 40px;margin-bottom:20px;border-bottom:1px solid #C4E8D6;">
					...
					<el-col :span="9">
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>供应商:</dt>
							<dd>{{ SIMdata.suppliername }}</dd>
						</dl>
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>状态:</dt>
							<dd>{{ SIMdata.status == 'INSTO' ? '在库' : SIMdata.status == 'INSTALL' ? '已安装' : SIMdata.status == 'LOST' ? '报失' : SIMdata.status == 'REPAIR' ? '维修' : SIMdata.tatus == 'DAMAGE' ? '报废' : SIMdata.status == 'ONWAY' ? '在途' : '' }}</dd>
						</dl>
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>协议:</dt>
							<dd>{{ SIMdata.protocoltype }}</dd>
						</dl>
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>安装位置:</dt>
					...
					<el-col :span="6">
						...
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>有效期:</dt>
							<dd>{{ SIMdata.expiredate == null ? "" :fmtdata.formatDate.format(new Date(SIMdata.expiredate), 'yyyy-MM-dd') }}</dd>
						</dl>
						<dl class="dllist" style="margin-bottom:10px;">
							<dt>备注:</dt>
							<dd>{{ SIMdata.remark }}</dd>
						</dl>
					</el-col>
				</el-row>
...
<script>
	...
	import { getProductInfoList, getSortList, getSelectListProd, getAllProSimInfoList, getSIMInfoList, getEmpGroupInfoList,getNamebyUserid } from '../../api/api';
	export default {
		...
		methods: {
			...
			formDetailHandle(simid, data) {
				...
				getAllProSimInfoList(para).then((res) => {
					...
					getSIMInfoList(paraTwo).then((res) => {
