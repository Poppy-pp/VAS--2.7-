...
<template>
	<section>
				...
				<el-row :gutter="10" class="colflex">
					...
					<el-col :span="5">
						<el-form-item prop="corpname" class="sf100">
							<el-select v-model="filters.corpname" @visible-change="changeCop" @change="changeVal" clearable filterable :loading="copLoading" placeholder="请选择操作公司">
		...
		<el-table :data="todo" stripe :max-height="windowOutHeight-350" border highlight-current-row @row-dblclick="handleEditRow" @expand-change="expandHandle" ref="todeTable" v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="id" align="center" label="维修单号">
			...
			<el-table-column prop="" align="center" label="车型">
				...
				</el-table-columnp="createTime" align="center" label="申请时间" :formatter="dateFormatter">
			</el-table-column>
					...
					<el-dropdown trigger="click">
						...
						<el-dropdown-menu slot="dropdown">
							...
							<template v-if="scope.row.name == '施工'">
								<!-- <el-dropdown-item @click.native="successWorkForm(scope.$index, scope.row)">完成维修</el-dropdown-item> -->
								<el-dropdown-item @click.native="refuseHandling(scope.$index, scope.row,0)">拒绝处理</el-dropdown-item>
							</template>
				...
				<el-col :span="19">
					<el-tabs v-model="dialogActiveNameImg">
						<el-tab-pane label="设备照片" name="1" class="deviceclas">
							<el-row :gutter="20">
								<template v-for="(item,index) in ruleFormStatic.afterSaleDetails">
									<el-col :span="24" v-if="item.newinstalldetail">
										<span class="formTile ml10">{{ item.newinstalldetail.packInfo.E_PRODTYPE+item.newinstalldetail.packInfo.E_PRODUNUM }}
                                </span>
										<el-col :span="6" class="photosh" v-for="(itemPic,ind) in item.newinstalldetail.pictures">
...
<script>
	...
	import { getTodoInfoList, getvariablesSeaviceTaskInfoList, getCorpNameList, getFormCount, addApprovaperSendVindicate, addApprovaperWork, addApprovaperServiceVerify, searchCorpNameList, getDeptManagerInfoList, getOperationRecord, addApprovaperVindicateCl } from '../../../api/api';
	export default {
		...
		methods: {
			...
			handleEdit(index, row) {
				this.$store.state.formObj = row;
				if(row.taskDefinitionKey == 'proc_vindicate_verify' || row.taskDefinitionKey == 'verify') {
					this.clerkOrderDialogTableVisible = true;
					this.clickState = 3;
					this.getFormData(row);
				}
				if(row.taskDefinitionKey == 'proc_vindicate_work' || row.taskDefinitionKey == 'work') {
					this.equipmentDialogTableVisible = true;
					this.clickState = 3;
					this.getFormData(row);
				}
			...
			initFormNum() {
				...
				getFormCount(para).then((res) => {
					...
					this.fromNum.num2 = (node.proc_vindicate_verify || node.verify) ? (node.proc_vindicate_verify || node.verify) : 0;
					this.fromNum.num3 = (node.proc_vindicate_work || node.work) ? (node.proc_vindicate_work || node.work) : 0;
					this.fromNum.num4 = (node.proc_vindicate_service_verify || node.service_verify) ? (node.proc_vindicate_service_verify || node.service_verify) : 0;
