...
<template>
	<section>
		<el-col :span="24" class="mb15">
			<el-radio-group v-model="stepState" @change="changeStateHandle">
				...
				<el-row :gutter="10" class="colflex">
					...
					<el-col :span="5">
						<el-form-item prop="corpname" class="sf100">
							<el-select v-model="filters.corpname" clearable @visible-change="changeCop" @change="changeVal" filterable :loading="copLoading" placeholder="请选择操作公司">
					...
					<el-col :span="4">
						<el-form-item prop="vehicle_modle" class="sf100">
							<el-input v-model="filters.vehicle_modle" @keyup.native.enter="getTodo" placeholder="请输入车辆型号"></el-input>
						</el-form-item>
					</el-col>
					<el-col :span="8">
		...
		<el-table :data="todo" stripe :max-height="windowOutHeight-350" border ref="todeTable" highlight-current-row @row-dblclick="handleEditRow" @expand-change="expandHandle" v-loading="listLoading" style="width: 100%;">
						...
						<el-tab-pane label="操作摘要" name="1">
							<el-row>
								...
								<el-col :span="24">
									<dl class="dllist">
										<dt>备注:</dt>
										<dd v-if="ruleFormStatic.flowData.start_remark">{{ ruleFormStatic.flowData.start_remark }}</dd>
									</dl>
								</el-col>
							</el-row>
						</el-tab-pane>
			...
			<el-table-column prop="id" align="center" label="拆除单号">
			...
			<el-table-column prop="" align="center" label="车型">
				<template scope="props">
					...
					<el-dropdown trigger="click">
						...
						<el-dropdown-menu slot="dropdown">
							...
							<template v-if="scope.row.name == '施工'">
								<!-- <el-dropdown-item @click.native="successWorkForm(scope.$index, scope.row)">完成拆除</el-dropdown-item> -->
								<el-dropdown-item @click.native="refuseHandling(scope.$index, scope.row,0)">拒绝处理</el-dropdown-item>
							</template>
...
<script>
	...
	import { getTodoInfoList, getvariablesSeaviceTaskInfoList, getCorpNameList, getFormCount, addApprovaperWork, addApprovaperServiceVerify, searchCorpNameList, getOperationRecord, addApprovaperSendVindicateRmove, deleteProcess, getDeptManagerInfoList, addApprovaperVindicate_removeCl } from '../../../api/api';
	export default {
		...
		methods: {
			...
			getTodo() {
				...
				getTodoInfoList(para).then((res) => {
					...
					if(res.data.result.code == 0) {
						...
						if(res.data.data.totalResult > this.fromNum.num0) {
							this.initFormNum();
						}
					}
				}).catch((error) => {
			...
			handleEdit(index, row) {
				this.$store.state.formObj = row;
				if(row.taskDefinitionKey == 'proc_vindicate_remove_node_verify' || row.taskDefinitionKey == 'node_verify') {
					this.clerkOrderDialogTableVisible = true;
					this.clickState = 3;
					this.getFormData(row);
				}
				if(row.taskDefinitionKey == 'proc_vindicate_remove_node_work' || row.taskDefinitionKey == 'node_work') {
					this.equipmentDialogTableVisible = true;
					this.clickState = 3;
					this.getFormData(row);
				}
			...
			initFormNum() {
				...
				getFormCount(para).then((res) => {
					...
					this.fromNum.num4 = (node.proc_vindicate_remove_work_verify || node.work_verify) ? (node.proc_vindicate_remove_work_verify || node.work_verify) : 0;
