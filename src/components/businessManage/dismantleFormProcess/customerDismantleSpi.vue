			...
			<el-form label-position="top" :model="ruleForm" ref="ruleForm" :rules="rule" class="cur-form-inn" label-width="110px" inline>
				<el-col :span="24" class="cbstyle">
					<el-collapse v-model="activeName" class="bdcoll">
							...
							<el-row :gutter="20">
								<el-col :span="6">
									<el-form-item label="车辆信息" prop="vinnum" class="wiredNum mt10">
										<el-autocomplete class="inline-input searchInput" popper-class="my-autocomplete" v-model="ruleForm.vinnum" :fetch-suggestions="querySearchVins" custom-item="my-item-zh" placeholder="请输入车牌号/车架号" @select="handleSelectVin"></el-autocomplete>
									</el-form-item>
								</el-col>
								<el-col :span="18">
						...
						<el-collapse-item title="设备信息" name="2" class="pb18 lefw10" v-show="isShow">
							<el-row :gutter="20">
										...
										<el-col :span="24" class="pl0">
											<el-col :span="6" class="pl0">
												<el-form-item label="拆除原因" class="mt5">
													<el-select v-model="ruleForm['temp'+index]" value-key="dictdatavalue" :loading="faultLoading" @visible-change="getFaultcodeInfo" filterable placeholder="请选择拆除原因">
								...
								<template v-else>
									<el-col :span="24" v-for="(items,index) in deviceData">
										<el-checkbox-group @change="checkChange" v-model="checkedCities1" class="mt10">
											<el-checkbox :disabled="items.isdelete == 1" :label="items">{{ items.e_prodnum }}({{ items.e_prodnum }}-{{ items.installpositionname }})</el-checkbox>
										</el-checkbox-group>
										<el-col :span="24" class="dec_carinfo ml0 pb10" v-show="checkedCities2.join(',').indexOf(items.packid) >= 0">
											<dl class="dllist">
												<dt>设备号：</dt>
								...
								<el-col :span="24">
									<el-col :span="6" class="pl0">
										<el-form-item label="拆除费用" prop="removeapplywireless" class="mt5">
						...
						<el-collapse-item title="拆除设备原因备注" name="4" class="pb18 lefw10">
							<el-row :gutter="20">
								<el-col :span="24">
									<el-form-item label="备注" prop="remark" class="mt18">
...
<script>
	...
	import { startVindicate_remove, getSearchVinNum, getCidSearchDevice, getRemoveTypeInfo, getRemoveDeclaration, getRemovInstallDetail } from '../../../api/api';
	//自定义搜索组件模板样式
