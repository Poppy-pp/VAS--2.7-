...
export default {
	...
	setCheckedTwo(tree, data, checked, childCheck) {
		if(childCheck) {
			for(var i = 0, len = data.corporateinfos.length; i < len; i++) {
				tree.setChecked(data.corporateinfos[i].id, checked);
				if(data.corporateinfos[i].corporateinfos.length > 0) {
					this.setCheckedTwo(tree, data.corporateinfos[i], checked, childCheck);
				}
			}
		} else {
			tree.setChecked(data.id, checked);
